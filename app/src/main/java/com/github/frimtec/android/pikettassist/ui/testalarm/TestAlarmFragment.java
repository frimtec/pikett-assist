...
public class TestAlarmFragment extends AbstractListFragment<TestAlarm> {
  ...
  TestAlarmFragment(DbFactory dbFactory) {
    super(TEST_ALARMS);
    ...
    return Optional.of(view -> {
      ...
      builder.setPositiveButton(R.string.general_ok, (dialog, which) -> {
        ...
        try (SQLiteDatabase db = this.dbFactory.getDatabase(WRITABLE)) {
          try (Cursor cursor = db.query(TABLE_TEST_ALARM_STATE, new String[]{TABLE_TEST_ALARM_STATE_COLUMN_ID}, TABLE_TEST_ALARM_STATE_COLUMN_ID + "=?", new String[]{newTestContext}, null, null, null)) {
            if (cursor.getCount() == 0) {
              ContentValues contentValues = new ContentValues();
              contentValues.put(TABLE_TEST_ALARM_STATE_COLUMN_ID, newTestContext);
              contentValues.put(TABLE_TEST_ALARM_STATE_COLUMN_LAST_RECEIVED_TIME, 0);
              contentValues.put(TABLE_TEST_ALARM_STATE_COLUMN_MESSAGE, getString(R.string.test_alarm_message_empty));
  ...
  private void deleteTestAlarm(TestAlarm selectedTestAlarm) {
    try (SQLiteDatabase db = this.dbFactory.getDatabase(WRITABLE)) {
      db.delete(TABLE_TEST_ALARM_STATE, TABLE_ALERT_COLUMN_ID + "=?", new String[]{String.valueOf(selectedTestAlarm.getContext())});
    }
    Set<String> superviseTestContexts = SharedState.getSuperviseTestContexts(getContext());
  ...
  private List<TestAlarm> loadTestAlarmList() {
    ...
    try (SQLiteDatabase db = dbFactory.getDatabase(READ_ONLY);
         Cursor cursor = db.query(TABLE_TEST_ALARM_STATE, new String[]{TABLE_ALERT_COLUMN_ID}, null, null, null, null, null)) {
